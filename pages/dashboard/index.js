...
export default function Dashboard(props) {
  ...
  const { user_data, categories } = props;
  ...
  const createBidItemDataRef = useRef();
  ...
  //   if (user.user?.address?.state) {
  //     setSateData(
  //       State.getStatesOfCountry(user.user.address?.country.split(",")[1])
  //     );
  //     console.log(user.user.address?.country.split(",")[1]);
  //   }
  // }, []);
  ...
  const onChangeItemImage = (e) => {
    if (e.target.files.length >= 5 || imgsSrc.length >= 4) {
      return alert("Gallery capacity exceeded");
    }
    ...
    for (const file of e.target.files) {
      ...
      reader.onload = async () => {
        await setImgsSrc((imgs) => [...imgs, reader.result]);
      };
      reader.onerror = () => {
  ...
  const [stateData, setSateData] = useState(null);
  ...
  const [createBidItemData, setCreateBidItemData] = useState({
    name: "",
    category: "",
    description: "",
    endsAt: "",
  ...
  const passErrorRef = useRef();

  const [addrError, setAddrError] = useState("");
  const addrErrorRef = useRef();

  const [profError, setProfError] = useState("");
  const profErrorRef = useRef();
  ...
  const changeUserPassword = async (e) => {
    ...
    if (formValues.password !== formValues.checkPassword) {
      passErrorRef.current.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
      return setPassError("password doesn't not match");
    }

    if (
      formValues.password.trim().length < 8 ||
      formValues.checkPassword.trim().length < 8
    ) {
      passErrorRef.current.scrollIntoView({
        behavior: "smooth",
        block: "start",
    ...
    try {
      const res = await fetch("/api/dashboard/changepassword", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (!res.ok) {
        setPassError((await res.json()).message);
        return;
      }
  ...
  const updateUserProfile = async (e) => {
    ...
    if (
      ...
      addrErrorRef.current.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
      return setProfError("Phone number is required.");
    }

    try {
      const res = await fetch("/api/dashboard/editprofile", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (!res.ok) {
        setProfError((await res.json()).message);
        return;
      }
  ...
  const updateUserAddress = async (e) => {
    ...
    if (
      !formValues.address.city === undefined ||
      !formValues.address.city === "" ||
      !formValues.address.line1 === undefined ||
      !formValues.address.line1 === "" ||
      !formValues.address.state === undefined ||
      ...
      return setAddrError("Address information incomplete");
    }

    try {
      const res = await fetch("/api/dashboard/changeaddress", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (!res.ok) {
        setAddrError((await res.json()).message);
        return;
      }
  ...
  // update user address
  const createBidItemDataFunc = async (e) => {
    e.preventDefault();
    setLoading(true);

    if (
      !createBidItemData.name ||
      !createBidItemData.category ||
      !createBidItemData.description ||
      !createBidItemData.endsAt
    ) {
      setLoading(false);
      return alert("Cannot create new Listing with insuffient information.");
    }

    try {
      const res = await fetch("/api/dashboard/create_new_item", {
        method: "POST",
        body: JSON.stringify({
          name: createBidItemData.name,
          category: createBidItemData.category,
          description: createBidItemData.description,
          endsAt: createBidItemData.endsAt.toString(),
          bidItemImage: imgsSrc,
          minBidAmount: createBidItemData.minBidAmount,
          startBidAmount: createBidItemData.startBidAmount,
          itemCondition: createBidItemData.itemCondition,
          sellersNote: createBidItemData.sellersNote,
        }),
        headers: {
          "Content-Type": "application/json",
        },
      ...
      let responsedata = await res.json().result;
      let oldListing = await userData?.listings;

      await setUserData({
        ...userData,
        listings: [responsedata, ...oldListing],
      });

      createBidItemDataRef?.current?.reset();

      alert("Auction listing created");
    } catch (error) {
      console.error(error);
    }
        ...
        <div className="row g-4">
          ...
          <div className="col-lg-3">
            ...
            >
              <button
                ...
                } nav-link nav-btn-style mx-auto  mb-20`}
              ...
              >
                ...
                Current Bidding List
              </button>
              <button
                className={`${
                  activeTab === "tab4" || activeTab === "tab6" ? "active" : ""
                } nav-link nav-btn-style mx-auto  mb-20`}
          ...
          <div className="col-lg-9">
            <div className="tab-content" id="v-pills-tabContent">
                  ...
                  <div className="row g-4">
                      ...
                      >
                        ...
                        <div className="body">
                          <div className="counter-item">
                            <h2>{user_data?.orders?.orderpending.length}</h2>
                          </div>
                          <div className="icon">
                            <svg
                              width="50"
                            ...
                            >
                              <g opacity="0.8">
                                ...
                                <path d="M12.8906 42.7246V43.5547H15.332H17.7832L17.7539 42.7441L17.7246 41.9434L15.3125 41.9141L12.8906 41.8945V42.7246Z" />
                                <path d="M12.959 45.1758C12.9199 45.2148 12.8906 45.5957 12.8906 46.0156V46.7773H15.332H17.7832L17.7539 45.9668L17.7246 45.166L15.3711 45.1367C14.082 45.127 12.998 45.1465 12.959 45.1758Z" />
                                <path d="M19.4043 45.1758C19.3652 45.2148 19.3359 45.5957 19.3359 46.0156V46.7773H20.166H21.0059L20.9766 45.9668L20.9473 45.166L20.2148 45.1367C19.8047 45.1269 19.4434 45.1367 19.4043 45.1758Z" />
                              </g>
                            </svg>
                          </div>
                        </div>
                    ...
                    <div className="col-md-6 col-sm-6">
                      ...
                      >
                        ...
                        <div className="body">
                          <div className="counter-item">
                            <h2>{user_data?.orders?.orderprocessing.length}</h2>
                          </div>
                          <div className="icon">
                            <svg
                              width="46"
                    ...
                    <div className="col-md-6 col-sm-6">
                      ...
                      >
                        ...
                        <div className="body">
                          <div className="counter-item">
                            <h2>{user_data?.orders?.orderpickedup.length}</h2>
                          </div>
                          <div className="icon">
                            <svg
                              width="50"
                    ...
                    <div className="col-md-6 col-sm-6">
                      ...
                      >
                        <div className="header">
                          <h5>Current Bidding List</h5>
                        </div>
                        <div className="body">
                          <div className="counter-item">
                            <h2>{user_data?.biddings.length}</h2>
                          </div>
                          <div className="icon">
                            <svg
                              width="50"
              ...
              >
                <p>
                  On updating any information you would be required to
                  authenticate your action
                </p>
                <div className="dashboard-profile">
                  {/* profile info */}
                  <div className="form-wrapper" ref={profErrorRef}>
                    <form action="#">
                      <div className="row">
                        ...
                        {/* <div className="col-xl-6 col-lg-12 col-md-6">
                          <div className="form-inner">
                            ...
                            >
                              <option
                                selected={
                                  !user.user.address?.state ? true : false
                                }
                              >
                                Select a State
                              </option>
                        ...
                        {/* <div className="col-xl-6 col-lg-12 col-md-6">
                          <div className="form-inner">
                            ...
                            <select
                              ...
                              placeholder="Your Country"
                              className="mb-2"
                              value={formValues.address.country.name}
                              onChange={(e) => {
                                ...
                                setSateData(State.getStatesOfCountry(val.code));
                              }}
                              defaultValue={user?.user?.address?.country}
                            >
                              <option
                                selected={
                                  !user.user.address?.country ? true : false
                                }
                              >
                                Select a Country
                              </option>
                                ...
                                ? countryData.map((country) => {
                                    ...
                                    return (
                                      <option
                                        ...
                                        value={value}
                                        selected={
                                          formValues.address.country.code === country.isoCode
                                            ? true
                                            : false
                                        }
                                      >
                  ...
                  {/* password */}
                  <div className="form-wrapper" ref={passErrorRef}>
                    <form action="#">
                      <div className="row">
                        <div className="col-12">
                          <div className="form-inner">
                        ...
                        <div className="col-12">
                          <div className="form-inner mb-0">
                            ...
                            <input
                              ...
                              name="password"
                              minLength={8}
                              defaultValue={formValues.checkPassword}
                              onChange={(e) =>
                                setFormValues({
                                  ...formValues,
              ...
              >
                ...
                <div className="table-wrapper">
                  <table className="eg-table order-table table mb-0">
                    ...
                    {userData?.biddings ? (
                      <tbody>
                        {userData.biddings.map((item) => {
                          const sortedBidders = [...item.bid.Bidders].sort(
                          ...
                          return (
                            <tr key={item.id}>
                              ...
                              <td data-label="Bidding ID">{item.id}</td>
                              <td data-label="Bid Amount(NGN)">
                                ₦{" "}
                                {(
                                  parseInt(item.amount) +
                                  parseInt(item.bid.startBidAmount)
                                )?.toLocaleString()}
                              </td>
                              <td data-label="Highest Bid">
                                ₦{" "}
                                {(
                                  parseInt(highestBidder.amount) +
                                  parseInt(item.bid.startBidAmount)
                                )?.toLocaleString()}
                              </td>
                              <td
              ...
              >
                <div className="table-title-area">
                  <h3>My Listings</h3>
                  <button
                    className="eg-btn p-3 action-btn"
                ...
                <div className="table-wrapper">
                  <table className="eg-table order-table table mb-0">
                    ...
                    {userData.listings ? (
                        ...
                        {userData.listings.map((listing, index) => {
                          return (
                            <tr key={listing?.id}>
                              ...
                              <td data-label="Bidding ID">{listing?.id}</td>
                              <td
                      ...
                      <p>No Listings Available create.</p>
                    )}
                  </table>
                </div>
              </div>
                ...
                <div className="table-wrapper">
                  <table className="eg-table order-table table mb-0">
                    ...
                    {userData?.sold ? (
                      ...
                      <p className="p-3">No Sold Items Yet Create a New Listing to start selling today.</p>
                    )}
                  </table>
                </div>
              </div>
                  ...
                  <div className="form-wrapper" ref={profErrorRef}>
                    <form action="#">
                      <div className="row">
                        ...
                        <div className="col-xl-6 col-lg-12 col-md-6">
                          <div className="form-inner">
                            <label>Bid Ends *</label>
                            <input
                              type="datetime-local"
                              onChange={(e) => {
                                setCreateBidItemData({
                                  ...createBidItemData,
                                  endsAt: ConvertDate(e.target.value),
                                });
                              }}
                            />
                          </div>
                        </div>
                        <div className="ccol-xl-12 col-lg-12 col-md-12">
                          <div className="form-inner">
                            <label>Sellers note *</label>
                            <input
                        ...
                        <div className="col-xl-12 col-lg-12 col-md-12">
                          <div className="form-inner">
                            ...
                            <textarea
                              ...
                              cols="6"
                              maxLength="800"
                              value={createBidItemData.description}
                        ...
                        <div className="col-xl-6 col-lg-12 col-md-6">
                          <div className="form-inner">
                            ...
                            <input
                              contentEditable={false}
                              type="number"
                              placeholder="Set a starting bid for your item"
                              value={createBidItemData.startBidAmount}
                              onChange={(e) => {
                                setCreateBidItemData({
                                  ...createBidItemData,
                                  startBidAmount: e.target.value,
                                });
                              }}
                            />
                          </div>
                        ...
                        <div className="col-xl-6 col-lg-12 col-md-6">
                          <div className="form-inner">
                            ...
                            <input
                              contentEditable={false}
                              type="text"
                              placeholder="Set a minimum bid amonut for your item"
                              value={createBidItemData.minBidAmount}
                              onChange={(e) => {
                                setCreateBidItemData({
                                  ...createBidItemData,
                          ...
                          {loading === false ? (
                            <div className="button-group">
                              <button
                                type="submit"
                                className="eg-btn profile-btn"
                                onClick={(e) => createBidItemDataFunc(e)}
                              >
                                Upload Listing Profile
                              </button>
                            </div>
                          ) : (
                            <p>Uploading new data, wait a munite</p>
                          )}
                        </div>
                      </div>
                    </form>
  ...
  // return "Permision denied. Please auhenticate your account and try again.";
}
